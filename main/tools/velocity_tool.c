...
static inline void set_gpio_velocity_tool()
{
    ...
    gpio_isr_handler_add(VELOCITY_TOOL_PIN, velocity_tool_interrupt_handler, (void *)VELOCITY_TOOL_PIN);
}
...
void velocity_tool_init(void *p1)
{
    ...
    while (true) {
        ...
        if(period_diff != 0 ) {
            ...
            velocity = rpm * 0.104719f; /* pi/30(conversão do RPM) * raio da roda * 3.6 */
            sprintf(payload, "velocity: %.5f", velocity);
            ESP_LOGI(TAG, "period %d ms rpm %f velocity %f", period_diff, rpm, velocity);
